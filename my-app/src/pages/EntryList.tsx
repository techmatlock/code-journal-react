...
// },
// ]
export function EntryList({setIsEditing}: Props) {
  const [entries, setEntries] = useState<Entry[]>([])
  const navigate = useNavigate();

  function handleClick(entryId: number) {
    setIsEditing(true);
    navigate(`/entry-form/${entryId}`);
  }
        ...
        <div className="row">
          <div className="column-full">
            <ul className="entry-ul" id="entryUl">
              {entries.map((entry)=> (<li key={entry.entryId} className="row">
                <div className="row justify-center column-half padding-lr">
                  ...
                  <div className="column-half padding-lr">
                    <p className="bold row justify-between">{entry.title}<FaPencil onClick={() => handleClick(entry.entryId)} className="edit"/></p>
                    <p>{entry.notes}</p>
                    </div>
                  </li>))}
            </ul>
